...
hook.Add("PopulateToolMenu", "GodSentToolsGraphicsProfile", function()
	spawnmenu.AddToolMenuOption("Utilities", "#godsenttools.name", "GodSentTools_Graphics_Profile", "#godsenttools.graphicsprofile.name", "", "", function(form)
		...
		reload:SetText("#godsenttools.graphicsprofile.refresh")
		reload:SetIcon("icon16/arrow_refresh.png")
		form:AddItem(reload)
		local list = vgui.Create("DListView")
		list:SetMultiSelect(false)
		list:AddColumn("#godsenttools.graphicsprofile.profiles")
		list:SetTall(100)
		form:AddItem(list)
		...
		apply:SetText("#godsenttools.graphicsprofile.select")
		...
		local function Run(name)
			...
			if not f then
				chat.AddText("#godsenttools.graphicsprofile.filenotfound")
				goto close
			end
		...
		function apply:DoClick()
			...
			local text = string.format(language.GetPhrase("#godsenttools.graphicsprofile.applying"), self.name)
		...
		function list:OnRowSelected(_, row)
			apply:SetText(string.format(language.GetPhrase("#godsenttools.graphicsprofile.run"), row:GetColumnText(1)))
